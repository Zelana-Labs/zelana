...
use crate::theme::*;
use crate::ui::{accounts::render_accounts, transactions::render_transactions, nullifiers::render_nullifiers};
...
pub fn render_ui(f: &mut Frame, app: &mut App) {
    ...
    let footer_right = Paragraph::new(status_text)
        .style(Style::default()
            .fg(if !app.status_message.is_empty() { COLOR_SUCCESS } else { COLOR_TEXT })
            .bg(COLOR_BG))
        .block(Block::default()
            .borders(Borders::TOP)
            .border_style(Style::default().fg(COLOR_BORDER).bg(COLOR_BG)));
