...
async fn main() -> anyhow::Result<()> {
    ...
    // Initialize State - Create executor ONCE
    let sessions = Arc::new(SessionManager::new());
    let executor = Arc::new(TransactionExecutor::new("./data/sequencer_db")?);
    info!("TransactionExecutor initialized with RocksDB");

    // Clone executor for indexer task
    let executor_indexer = executor.clone();
    tokio::spawn(async move {
        ...
        // Use db from the shared executor
        ingest::start_indexer(executor_indexer.db.clone(), wss_url, bridge_id).await;
    });

    // Clone executor for HTTP API server
    let executor_api = executor.clone();
    tokio::spawn(async move {
        // Pass both db AND executor to the ingest server
        ingest::state_ingest_server(executor_api.db.clone(), executor_api, 8080).await;
    });
    ...
    loop {
        ...
        match Packet::parse(packet_data) {
            ...
            Ok(Packet::AppData { nonce, ciphertext }) => {
                ...
                match decrypted_opt {
                    Some(Ok(plaintext)) => {
                        // Handle Transaction using shared executor
...
fn verify_signature(tx: &SignedTransaction) -> anyhow::Result<()> {
    ...
    let vk = VerifyingKey::from_bytes(&tx.signer_pubkey.0)?;
    let sig = Signature::from_slice(&tx.signature.0)?;
