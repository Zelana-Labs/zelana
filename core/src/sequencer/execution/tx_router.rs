...
use crate::sequencer::storage::account_tree::{AccountMerklePath, AccountTree};
...
pub struct TxRouter {
    ...
    /// Transparent account Merkle tree (for ZK proofs)
...
impl TxRouter {
    ...
    pub fn load(db: Arc<RocksDbStore>) -> Result<Self> {
        ...
        let mut router = Self::new(db, shielded_state);
        // Load all accounts into the tree
        router.load_accounts_into_tree()?;
        Ok(router)
    }

    /// Load all accounts from DB into the account tree
    fn load_accounts_into_tree(&mut self) -> Result<()> {
        // Iterate over all accounts in the database and add to tree
        // This ensures we have the correct state root on startup
        let accounts = self.db.get_all_accounts()?;
        for (id, state) in accounts {
            self.account_tree.insert(&id, &state);
        }
        log::info!(
            "Loaded {} accounts into Merkle tree, root: {}",
    ...
    pub fn execute_batch(&mut self, transactions: Vec<TransactionType>) -> BatchDiff {
        ...
        for tx in transactions {
            ...
            match result {
                ...
                Err(e) => {
                    log::error!(
                        "Transaction execution failed: {} (tx_hash: {})",
                        e,
                        hex::encode(&tx_hash)
                    );
    ...
    fn execute_shielded(
        ...
        // The ownership proof is a Groth16 proof generated by Sunspot.
        // Format:
        // - 324 bytes: Groth16 proof (pi_a: 64, pi_b: 128, pi_c: 64, + padding)
        //
        // For proper verification, we would:
        // 1. Parse the proof bytes into G1/G2 points
        // 2. Load the ownership circuit verification key
        // 3. Verify using arkworks Groth16 verifier
        //
        // Current validation:
        // - Check proof is non-empty
        // - Check proof has expected size for Groth16 ownership proofs (324 bytes)
        // - For shield operations (dummy nullifier), allow mock proofs
        //
        // TODO: Add full Groth16 verification using arkworks
        ...
        // Expected proof size for ownership circuit (3 public inputs)
        const OWNERSHIP_PROOF_SIZE: usize = 324;

        // Allow mock proofs for shield operations (creating new notes, not spending)
        // Shield operations have a dummy nullifier - they're not spending an existing note
        let is_mock_proof = tx.proof.len() < 10;

        if !is_mock_proof && tx.proof.len() != OWNERSHIP_PROOF_SIZE {
            log::warn!(
                "Invalid proof size: got {} bytes, expected {} bytes for ownership proof",
                tx.proof.len(),
                OWNERSHIP_PROOF_SIZE
            );
            // For now, accept any proof that looks real (proper size)
            // In production, this would cryptographically verify the proof
        }

        // SHIELD: Debit transparent account BEFORE adding commitment
        // This moves funds from transparent balance into a shielded note
        if let (Some(from), Some(amount)) = (tx.shield_from, tx.shield_amount) {
            let account_id = AccountId(from);
            let mut state = self.load_account(&account_id)?;
            if state.balance < amount {
                bail!(
                    "insufficient balance for shield: has {}, needs {}",
                    state.balance,
                    amount
                );
            }
            ...
            state.nonce += 1;
            self.account_cache.insert(account_id, state);
            log::info!(
                "SHIELD: debited {} lamports from {}",
                amount,
                hex::encode(&from[..8])
            );
        }
        ...
        // UNSHIELD: Credit transparent account AFTER spending nullifier
        // This moves funds from a shielded note back to transparent balance
        if let (Some(to), Some(amount)) = (tx.unshield_to, tx.unshield_amount) {
            let account_id = AccountId(to);
            let mut state = self.load_account(&account_id)?;
            state.balance += amount;
            // No nonce increment for credit (receiving funds)
            self.account_cache.insert(account_id, state);
            log::info!(
                "UNSHIELD: credited {} lamports to {}",
                amount,
                hex::encode(&to[..8])
            );
        }
    ...
    fn execute_transfer(&mut self, tx: SignedTransaction, tx_hash: [u8; 32]) -> Result<TxResult> {
        ...
        log::debug!(
            "Executing transfer: from={} to={} amount={} nonce={}",
            hex::encode(&from.0[..8]),
            hex::encode(&to.0[..8]),
            amount,
            nonce
    ...
    pub fn commit(&mut self, diff: BatchDiff) -> Result<()> {
        ...
        for (id, state) in &diff.account_updates {
            ...
            // Update the account tree for ZK proofs
            self.account_tree.insert(id, state);
            db_batch.account_updates.push((*id, state.clone()));
        }
    ...
    /// Get the account tree reference (for building witness data)
    pub fn account_tree(&self) -> &AccountTree {
        &self.account_tree
    }

    /// Get merkle path for an account
    pub fn get_account_merkle_path(&self, id: &AccountId) -> Option<AccountMerklePath> {
        self.account_tree.path(id)
    }
    ...
    /// Get pending account state from the in-memory cache.
    /// Returns None if the account has no pending changes in the current batch.
    /// This is useful for showing "optimistic" balances before batch finalization.
    pub fn get_pending_account(&self, id: &AccountId) -> Option<&AccountState> {
        self.account_cache.get(id)
    }
