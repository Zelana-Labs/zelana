...
impl Settler {
    ...
    ///
    /// Noir proofs can be either:
    /// - 388 bytes (just proof)
    /// - 624 bytes (proof + public witness, from noir_client)
    pub fn is_noir_proof(proof: &BatchProof) -> bool {
        proof.proof_bytes.len() == NoirProofData::PROOF_SIZE
            || proof.proof_bytes.len() == NoirProofData::COMBINED_SIZE
    }
    ...
    ///
    /// For Noir/Sunspot proofs, uses direct verification (bypasses Bridge program)
    /// to avoid CPI complexity. The Bridge state update happens separately.
    pub fn submit_proof_auto(
        ...
        if Self::is_noir_proof(proof) {
            tracing::info!(
                "Detected Noir/Sunspot proof ({} bytes), using direct Sunspot verification",
                proof.proof_bytes.len()
            );
            // Use direct verification to avoid Bridge CPI issues
            self.verify_sunspot_direct(proof)
        } else {
            tracing::info!(
                "Detected Groth16 proof ({} bytes), using arkworks verifier via Bridge",
                proof.proof_bytes.len()
            );
...
impl NoirProofData {
    ...
    /// Combined proof + public witness size (used by noir_client)
    pub const COMBINED_SIZE: usize = Self::PROOF_SIZE + Self::PUBLIC_WITNESS_SIZE; // 624
    ...
    ///
    /// Handles two formats:
    /// - 388 bytes: Just proof, public witness from BatchPublicInputs
    /// - 624 bytes: Combined proof + public witness (from noir_client)
    pub fn from_batch_proof(proof: &BatchProof) -> Result<Self> {
        if proof.proof_bytes.len() == Self::COMBINED_SIZE {
            // Combined format: first 388 bytes = proof, last 236 bytes = public witness
            let proof_bytes = proof.proof_bytes[..Self::PROOF_SIZE].to_vec();
            let public_witness = proof.proof_bytes[Self::PROOF_SIZE..].to_vec();

            Ok(Self {
                proof_bytes,
                public_witness,
            })
        } else if proof.proof_bytes.len() == Self::PROOF_SIZE {
            ...
            bail!(
                "proof is not a Noir proof: {} bytes (expected {} or {})",
                proof.proof_bytes.len(),
                Self::PROOF_SIZE,
                Self::COMBINED_SIZE
            );
