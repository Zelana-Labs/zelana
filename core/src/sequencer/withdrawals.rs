...
impl WithdrawalQueue {
    ...
    pub fn load(db: Arc<RocksDbStore>) -> Result<Self> {
        ...
        // Note: In production, we'd iterate over CF_WITHDRAWALS
        // For MVP, we start fresh on restart (pending withdrawals should be
        // in the batch queue or already submitted to L1)
