...
use crate::{
    ...
    state::{DepositReceipt, Vault},
    ID,
};

pub fn process_deposit(accounts: &[AccountInfo], ix_data: &[u8]) -> ProgramResult {
    let [depositor, config_account, vault_account, deposit_reciept_account, _system_program] =
        accounts
    else {
    ...
    let receipt_state = unsafe { load_acc_mut_unchecked::<DepositReceipt>(&mut receipt_data)? };
